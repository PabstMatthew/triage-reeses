...
#define LLC_LATENCY 12  // 4 (L1I or L1D) + 8 + 20 = 32 cycles
...
class CACHE : public MEMORY {
  public:
    ...
    int  check_hit(PACKET *packet),
         ...
         prefetch_line(uint64_t ip, uint64_t base_addr, uint64_t pf_addr, int prefetch_fill_level, uint64_t prefetch_metadata),
         kpc_prefetch_line(uint64_t base_addr, uint64_t pf_addr, int prefetch_fill_level, int delta, int depth, int signature, int confidence, uint64_t prefetch_metadata);
    ...
    void add_mshr(PACKET *packet),
         ...
         l1d_prefetcher_cache_fill(uint64_t addr, uint32_t set, uint32_t way, uint8_t prefetch, uint64_t evicted_addr, uint64_t metadata_in),
    ...
    int get_metadata(uint64_t phy_addr);
    int write_metadata(uint64_t phy_addr);
    void complete_metadata_req(uint64_t phy_addr);
